<template lang="en">
    <div>
        ...
        <ArticleAr v-for="(article, index) in this.$store.state.articles" :key="article.id" :article="article" :index="index">
        </ArticleAr>

        <ArticleDelete v-for="(article, index) in this.$store.state.articles" :key="article.id" :article="article" :index="index"></ArticleDelete>
...
export default {
    ...
    methods: {
        addArticle() {
            if (this.newEnTitle.trim().length == 0 || this.newEnArticle.trim().length == 0) {
                return
            }
